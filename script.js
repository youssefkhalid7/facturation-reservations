async function processExcel() {
  ...
  reader.onload = async function (e) {
    ...
    for (const row of rows.filter(r => r.Status === 'OK')) {
      ...
      draw(row["Reservation number"], 60, 470, 12);

      // Check-in (droite)
      rightAlign(row["Arrival"], pageWidth - 60, 470, 12);

      // Check-out (droite)
      rightAlign(row["Departure"], pageWidth - 60, 440, 12);

      // Guest information (gauche)
      const guestName = (row["Guest name"] || "").split(/[\r\n]+/)[0];
      draw(guestName, 60, 440, 12);

      // Total guests (gauche)
      draw(row["Persons"], 60, 410, 12);

      // Total units/rooms (gauche)
      draw(row["Rooms"], 60, 380, 12);

      // Length of stay (droite)
      rightAlign(`${row["Room nights"]} night${parseInt(row["Room nights"]) > 1 ? "s" : ""}`, pageWidth - 60, 410, 12);

      // Total unit/room price (en bas, gras)
      rightAlign(`€ ${parseFloat(row["Final amount"]).toFixed(2)}`, pageWidth - 60, 220, 14, helveticaBold);
